  ...
  ({ className = "", children, ...props }, ref) => {
    return (
      <button
        ref={ref}
        type="button"
        className={`flex h-9 w-full items-center justify-between rounded-md border border-gray-300 bg-white px-3 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
        {...props}
      >
        {children}
...
const SelectContent: React.FC<SelectContentProps> = ({ className = "", children }) => {
  return (
    <div className={`absolute z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border bg-white shadow-md ${className}`}>
